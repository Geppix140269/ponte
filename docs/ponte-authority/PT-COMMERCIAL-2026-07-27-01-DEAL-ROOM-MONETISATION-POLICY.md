...
when credible interest moves to the next step: structured transaction
progression inside a Deal Room.

> The upstream market creates liquidity. The Deal Room captures value.

The Deal Room is therefore both:
...
express credible interest and reach a proposed Deal Room without encountering a
paywall that prevents Ponte from building market liquidity.

A proposed room may show:

- the commercial subject;
- the proposed participants and roles;
...
The exact admission and payment sequence remains to be designed, but the
commercial rule is fixed:

> A standard active Deal Room is not a permanently free product.

## 3. Permitted entitlement sources

An active room may be entitled through:

- payment by the room initiator;
- payment by one principal party;
- payment shared by multiple or all principal parties;
- payment or sponsorship by an association, institution or programme;
- a subscription that explicitly includes defined Deal Room capacity;
...
Payment, participation, visibility and decision authority remain separate.

## 4. Monetisation around the room

Ponte may monetise value at different points around a Deal Room, including:

- room activation;
- organisation or participant access;
...
- specialist inspection, logistics, compliance, legal, finance or other
  professional support coordinated through the room;
- portfolio plans covering multiple active rooms;
- transaction, referral, completion or success fees where attribution, legality,
  payment and operational rules are clear.

These are allowed commercial model families, not approved prices or packages.

All primary launch revenue mechanisms must be anchored to a Deal Room, a
proposed Deal Room or a clearly defined service that supports entry into or
progress through a Deal Room.

## 5. Relationship to Ponte Desk
...
A self-managed or agent-assisted Deal Room remains a monetised product even when
no Ponte human participates.
...
- **Required product capability:** a commercial-entitlement gate for Deal Room
  activation, including paid, included, sponsored, promotional or waived states.
...
The exact billing implementation is separately approved later.

A payment or entitlement failure must not destroy the proposed room, the user's
submitted work or the durable audit record. The product must define what can be
viewed, resumed or exported in each state.
...
```text
Market Signals, Member Opportunities and member activity
        ↓
Discover and structure commercial intent
        ↓
Establish credible commercial interest
        ↓
Propose the Deal Room and show admission/commercial terms
        ↓
Obtain the required entitlement
        ↓
Activate the Deal Room
        ↓
Monetise transaction progression and optional assisted services
        ↓
Record the outcome and support further rooms
```
...
- Do not let the payer obtain disclosure, approval or decision rights that were
  not separately granted.
...
- Do not begin substantive human or specialist work without entitlement and
  accepted scope.
- Do not invent prices, discounts, package limits, transaction fees or success
  fees before owner approval.
- Do not use a generic low-cost subscription to imply unlimited Deal Rooms or
  uncapped Ponte Desk access.
...
- who pays in each room type;
- what is visible before entitlement;
- whether payment occurs before invitations, before admission or before active
  procedure access;
- what happens when another required participant declines;
- room activation fees and included participant capacity;
- self-managed, agent-assisted and human-assisted package boundaries;
- subscriptions for multiple or recurring rooms;
- sponsored and institutional models;
- promotional and founding-member rules;
- refunds, credits, abandonment, expiry and disputes;
- specialist-service commercial treatment;
- transaction or success-fee conditions;
- Stripe replay safety, invoices and taxes;
...
The commercial model must be completed and approved before implementation.

...
5. **"Open the room" with no price context.** It is **Create a Deal Room, free**, then **Activate**. Where a price appears on this path at all, it is stated per `ADR-0029`: **$79 USD for 30 calendar days, five active counterparty branches included, first activation free while it has one active branch.** Read every number from `lib/deal-room/pricing.ts`. Never type one into a surface.

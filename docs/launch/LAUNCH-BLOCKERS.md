...
`LB-007` is **reserved for PR #107**, which currently calls its Market Signals
search blocker `LB-005`. That number is taken: `LB-005` is the
`20260729b_deal_room_rls.sql` grant-signature defect found by Gate C Approval 1.
Owner decision of 30 July 2026: **before PR #107 can merge, renumber its blocker
to `LB-007`** and correct every reference to it on that branch.

This is the third register collision between concurrent branches. An identifier
invented on an unmerged branch is not reserved until it merges, so a branch that
sat unpushed while another landed will always risk one. The cheap habit that
prevents it is to re-read this file against `origin/main` immediately before

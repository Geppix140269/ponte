...
### LB-014 — CLASSIFIED BY THE OWNER — The public fee page sells a commercial model the owner has retired, and never names the only paid product
...
| **Classification** | **Classified by the owner on 7 August 2026 (decision OD-F): `/pricing` must be reconciled to the current Deal-Room-only commercial model.** Previously PROPOSED and unclassified. ADR-0036 widens where the Deal Room may be publicly explained, but does **not** release `/pricing` from naming it — `PT-COMMERCIAL-2026-07-31-01` section 19 still requires that, and the requirement now sits beside a landing which already states the model correctly while the fee page contradicts it. **Not yet implemented.** |
...
### LB-015 — CLASSIFIED BY THE OWNER — A public page demonstrates an owner-review control that does not exist

| Field | Record |
|---|---|
| **Discovered** | 2026-08-07, during the Recovery Mode authority reconciliation |
| **Core journey or system** | The public Deal Room walkthrough at `/deal-rooms/inside`, and by reference JR-01 step 9 (owner review) |
| **Classification** | **Surface-specific release blocker, classified by the repository owner on 7 August 2026.** It publicly represents a capability that does not currently exist |
| **Owner** | Giuseppe Funaro |
| **Evidence** | `components/deal-room/JoinInbox.tsx` renders an inbox with **`Accept` / `Clarify` / `Decline`** controls (lines ~119–153). It is a static mock: it posts nowhere. It is rendered by `components/deal-room/Walkthrough.tsx` inside `app/[locale]/deal-rooms/inside/page.tsx`, which is **public, unflagged, with no session and no allowlist**. The real owner-review surface is `/workspace`, and it implements **`accepted` and `declined` only** — `connectDecisionAction` in `app/[locale]/_actions/listings.ts:252` has no clarification path. Canonical `O04` in `00-MASTER-IMPLEMENTATION-BRIEF.md` section 9.12 requires "Accept, decline or ask for clarification"; the register records the gap at JR-01 field 14 |
| **Why it blocks** | `00-NORTH-STAR-ENTRY-ARCHITECTURE.md` section 3.5 forbids any active interface promising a capability that is not "implemented, enabled, reachable and complete". ADR-0036 authorises the Deal Room to be publicly explained and demonstrated, and section 3.5 applies in full to every surface it permits — so widening the public surface widened the reach of this misrepresentation rather than excusing it. `PT-PRODUCT-2026-07-27-01` section 15 additionally requires illustrative material to be explicitly illustrative |
| **Scope** | **Surface-specific.** It does not block a member from completing a journey and creates no security or data-integrity risk. It is a truthful-representation defect on one public page |
| **Resolution options** (owner's choice, not an agent's) | (a) build the clarification action, closing the canonical `O04` gap as well; (b) remove `Clarify` from the mock so it depicts only what exists; or (c) mark the walkthrough unmistakably illustrative. Options (b) and (c) are copy-and-markup only |
| **Status** | **Open. No application change made.** Recovery Mode permits break/fix only where it restores already-approved behaviour, and every option above is a product decision |

## Reserved identifiers

**Next free identifier: `LB-016`.**

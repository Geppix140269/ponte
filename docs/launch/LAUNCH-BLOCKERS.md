...
| LB-008 | `anon` holds EXECUTE on every Deal Room function, including the one event logger that has no authorisation check of its own | 2026-07-30 | PROGRESS: the integrity of the Deal Room activity record, which is the append-only history every other guarantee is audited against | Found by the Gate C Approval 1 continuation, immediately after `20260729b` was applied. `20260729b` states its intent at the grant block: "`anon` is granted execute on nothing. The event logger is revoked from everyone: a member who could call it directly could forge history, which is the whole reason the other commands write it for them." The file then performs `revoke all on function public.deal_room_log_event(...) from public`. That revoke works - PUBLIC is absent from the function ACL - but it is the wrong target. Supabase `alter default privileges` grants EXECUTE **explicitly to `anon`, `authenticated` and `service_role`** on every new function in `public`, and revoking from PUBLIC does not touch an explicit role grant. The ACL reads `postgres=X`, `anon=X`, `authenticated=X`, `service_role=X`. All 23 `deal_room_*` functions are anon-executable. `deal_room_log_event()` is the one that matters: it is deliberately unauthorised internally, because the other commands call it on the member's behalf, so the grant was its only protection. It inserts whatever it is given into `deal_room_activity_events`, stamping `auth.uid()` as actor. **Proved through the public API, not inferred from the catalogue:** an anon-key RPC call returned `409 / 23503`, a foreign-key violation naming the `room_id` passed in, which means the body executed. The other commands returned `401 / 42501 Not authenticated`, so they refuse anon inside the body - defence in depth working, but not the grant boundary the file claims. Evidence: `docs/codex/audits/deal-room/GATE-C-APPROVAL-1-2026-07-30.md` section 7 | Repository owner | **Open, and narrowed. The corrective migration was applied to production on 30 July 2026 at 07:59:45.928 UTC** (`20260730b_deal_room_function_acl.sql`, SHA-256 `15f488d87705e5a88def6e1c25e0b006daceda9d3316747eb8bbe87b3f542b31`, one transaction, exit 0, ledger 45 to 46, checksum matching byte for byte). **The anonymous execution path is closed and proved closed through a real client:** `anon` holds EXECUTE on **0 of 23** `deal_room_*` functions, down from 23; `PUBLIC` holds 0; `deal_room_log_event` is reachable by `postgres` and `service_role` only; and an anon-key RPC to the logger now returns `401 / 42501 permission denied for function deal_room_log_event` where it previously returned `409 / 23503`, the foreign-key violation that proved the body was executing. **It stays open because the ACL contract is not yet what was specified.** `authenticated` holds EXECUTE on 22 functions, not 19: the 19 intended plus `deal_room_is_writable`, `deal_room_uuid_or_null` and `deal_room_events_append_only`. The migration revokes `authenticated` only on the logger, and re-granting the 19 cannot remove grants Supabase default privileges had already created on all 23. **Residual exposure, stated precisely:** all three are closed to `anon`; `is_writable` is a read-only boolean predicate, `uuid_or_null` is pure text coercion with no table access, and `events_append_only` raises if called outside a trigger context because it dereferences `OLD`/`NEW`. **None of them writes, and none is the forgery path** - the one function that could forge the append-only history is closed to both member roles. So the material security objective is met and the contract is not. **Probe 6 is pending, not passed:** a real authenticated direct RPC needs a member JWT, there are no authorised test credentials, and production 9 confirmed users are real member accounts - minting a session for one to satisfy a probe is not a test credential and was not done. Probes 8 and 9 are catalogue-verified and behaviourally pending for the same reason | #117 (repository); production application recorded in the Gate C audit sections 11 to 16 | **Applied and verified; 8 probes passed, 1 failed, 1 pending, 2 partial.** **Passed:** exactly one ledger row with matching checksum; `anon` EXECUTE on zero; `PUBLIC` EXECUTE on zero; `authenticated` cannot execute the logger; a real anonymous RPC to the logger refused with `42501` rather than the prior `23503`; `service_role` unchanged at 23; and **nothing else moved** - all nine before/after md5 fingerprints identical across function bodies, policy definitions, triggers, indexes, constraints, columns, RLS state and `pg_default_acl` (24 rows), with 14 policies, 4 agreement documents, 0 rooms and 0 activity rows. A before-baseline was captured deliberately so that "unchanged" could be proved rather than asserted. **Also confirmed:** anonymous calls to the commands and helpers now fail at the grant (`42501 permission denied for function ...`) rather than inside the body (`42501 Not authenticated`), so the refusal has moved to where it belongs; and member table reads still return `200 []` rather than erroring, which confirms against the live database that revoking the RLS helpers from `anon` did not break policy evaluation - the 14 policies are scoped `to authenticated`, so for an anonymous request no policy applies and the helpers are never called. **Failed:** probe 7, `authenticated` at 22 rather than 19, and probe 10 for three of its four functions. **Pending:** probe 6. **Partial:** probes 8 and 9, catalogue-verified only. **The regression suite did not catch the failure, and its wording overstates its reach.** `lib/deal-room/__tests__/function-acl.test.ts` asserts "`authenticated` should end with execute on exactly 19" and passes, because it counts `grant` statements in the file. A file-text test cannot see a privilege the file never mentions. That is the LB-008 error one level up: LB-008 was a file asserting something about itself, and the test written to catch it asserts something about that file. Only the catalogue could answer it, and it was not consulted until after the migration was applied. Correcting both the test and the ACL is a fix, and no fix was authorised in this pass. **What closes this blocker:** a follow-up migration revoking `authenticated` on the three internal functions, and probe 6, which needs an authorised test account. Evidence: `docs/codex/audits/deal-room/GATE-C-APPROVAL-1-2026-07-30.md` sections 11 to 16 |

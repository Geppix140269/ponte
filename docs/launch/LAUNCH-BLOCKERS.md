...
| LB-008 | `anon` holds EXECUTE on every Deal Room function, including the one event logger that has no authorisation check of its own | 2026-07-30 | PROGRESS: the integrity of the Deal Room activity record, which is the append-only history every other guarantee is audited against | Found by the Gate C Approval 1 continuation, immediately after `20260729b` was applied. `20260729b` states its intent at the grant block: "`anon` is granted execute on nothing. The event logger is revoked from everyone: a member who could call it directly could forge history, which is the whole reason the other commands write it for them." The file then performs `revoke all on function public.deal_room_log_event(...) from public`. That revoke works - PUBLIC is absent from the function ACL - but it is the wrong target. Supabase `alter default privileges` grants EXECUTE **explicitly to `anon`, `authenticated` and `service_role`** on every new function in `public`, and revoking from PUBLIC does not touch an explicit role grant. The ACL reads `postgres=X`, `anon=X`, `authenticated=X`, `service_role=X`. All 23 `deal_room_*` functions are anon-executable. `deal_room_log_event()` is the one that matters: it is deliberately unauthorised internally, because the other commands call it on the member's behalf, so the grant was its only protection. It inserts whatever it is given into `deal_room_activity_events`, stamping `auth.uid()` as actor. **Proved through the public API, not inferred from the catalogue:** an anon-key RPC call returned `409 / 23503`, a foreign-key violation naming the `room_id` passed in, which means the body executed. The other commands returned `401 / 42501 Not authenticated`, so they refuse anon inside the body - defence in depth working, but not the grant boundary the file claims. Evidence: `docs/codex/audits/deal-room/GATE-C-APPROVAL-1-2026-07-30.md` section 7 | Repository owner | **The defect is fixed in production. The blocker stays open on one outstanding verification only.** Two migrations closed it. `20260730b` (applied 07:59:45.928 UTC) took EXECUTE from `PUBLIC` and `anon` on all 23 `deal_room_*` functions and from `authenticated` on the event logger. `20260730c_deal_room_internal_acl.sql` (SHA-256 `5adb34c2ef183c601b30048084121577cf65cba29ad4fb7dacb075ac8c7d1891`, applied **08:26:17.995 UTC**, ledger 46 to 47, one transaction, exit 0) took the three `20260730b` had left - `deal_room_is_writable`, `deal_room_uuid_or_null`, `deal_room_events_append_only` - away from `authenticated` as well, because granting the 19 cannot remove grants Supabase default privileges had already written onto all 23. **Production now reports exactly the intended contract:** `anon` 0 of 23, `PUBLIC` 0, `authenticated` **19 of 23 matched by name**, `service_role` 23 unchanged, and all four internal functions reachable by `service_role` alone. **What remains is not a defect but a probe:** the real authenticated direct-RPC confirmation, which needs a dedicated QA account that does not yet exist. The catalogue already proves `authenticated` cannot execute the logger, and the same enforcement is proved at the API layer for `anon` (`42501` on the identical function), so the residual uncertainty is whether PostgREST enforces for `authenticated` what it demonstrably enforces for `anon`. Kept open per the owner standing instruction not to claim LB-008 fully resolved while that client is unavailable | #117, #123 (repository); production application recorded in the Gate C audit | **Verified in production, and by the right instrument this time.** `npm run deal-room:acl-verify` (`scripts/deal-room-acl-verify.mjs`) reads `pg_proc.proacl` and requires `anon` 0, `PUBLIC` 0, `authenticated` exactly the 19 **by name** - surplus and missing both fail, because losing an RLS helper breaks member journeys rather than the contract - `service_role` 23 unchanged, the four internal functions unreachable by either member role, 21 SECURITY DEFINER functions all with a pinned `search_path`, and 14 policies with none non-SELECT and none naming `anon`. **It exits 0.** **Demonstrated in both directions, not asserted:** run against production before `20260730c` it exits 1 with five problems, naming all three functions and reporting `authenticated 22 of 23, expected 19`. **Nothing else moved:** all nine before/after md5 fingerprints identical across function bodies, policy definitions, triggers, indexes, constraints, columns, RLS state and `pg_default_acl` (24 rows); 14 policies, 4 agreement documents, 0 rooms, 0 activity rows, no `deal-room-evidence` bucket. Ledger increased by exactly one row with the correct checksum. **A real anonymous client confirms the end state end to end:** the logger, the commands and the helpers all return `401 / 42501 permission denied for function ...`, while member table reads still return `200 []` rather than erroring - so revoking the three did not disturb policy evaluation. **The instrument problem is also fixed, which is the durable part.** `function-acl.test.ts` previously asserted "authenticated should end with execute on exactly 19" and passed while production held 22, because it counted `grant` statements in a file. Every assertion now states whether it is a claim about the file or about the world, the false message is replaced, and one of its 28 assertions fails if the verification script stops existing or stops interrogating all three roles - so the end state cannot silently lose its only witness. Evidence: `docs/codex/audits/deal-room/GATE-C-APPROVAL-1-2026-07-30.md` |

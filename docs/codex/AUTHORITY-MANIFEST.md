...
| 1c | `docs/ponte-authority/PT-COMMERCIAL-2026-07-27-01-DEAL-ROOM-MONETISATION-POLICY.md` | Ponte Trade Deal Room Monetisation Policy | **Accepted 27 July 2026; effective on merge.** The upstream market creates liquidity; the active Deal Room is Ponte's primary paid commercial environment. This policy amends the broad MVP payment exclusion so that only trade settlement, escrow and party-to-party transaction payments remain excluded. A Deal Room entitlement gate is required product capability, but prices, billing, Stripe, schema and runtime implementation remain separately unapproved. |
...
| 1e | `docs/ponte-authority/PT-COMMERCIAL-2026-07-27-02-DEAL-ROOM-LAUNCH-PRICING-V1.md` | Ponte Trade Deal Room Launch Pricing and Entitlement Model v1 | **Proposed for owner approval.** Defines the day-one subscription and credit configuration: five concurrent master rooms, unlimited related sub-rooms, sponsored guest allowances and pay-as-you-go credits. Numerical prices and allowances are not binding until accepted. |
...
The Master Implementation Brief states that it is the single self-contained authority and that Codex must not assume any other document is available. It therefore governs when it conflicts with older material, except where the North Star entry authority, the Deal Room Product Contract, the Deal Room Monetisation Policy, the accepted Deal-to-Room and Sub-Room Model within their scopes, or a later accepted ADR explicitly supersedes a named decision, or where a verified live technical or legal constraint is discovered and reported.

The proposed launch-pricing authority does not become binding until owner approval. Its existence on a branch is not implementation status and does not authorise charging.

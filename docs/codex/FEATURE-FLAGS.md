...
| `NEXT_PUBLIC_DEAL_ROOM_MULTILINGUAL` | **Proposed (LB-009), not yet wired.** `on` will enable multilingual message translation and interpretation presentation inside the Deal Room; anything else hides the derived surfaces while preserving original messages and confirmed canonical terms | Off | **Not set. Not implemented in production.** | Unset it and redeploy: translation and interpretation stop, originals and confirmed terms stay readable. |
| `DEAL_ROOM_TRANSLATION_ENABLED` | **Proposed (LB-009), server-only.** `on` (with an Anthropic key) opens the provider boundary so real translation calls may run. Off means the adapter is inactive and sends **no** content, returning an honest `provider_unavailable` status | Off (inactive) | **Not set. Gated by OD-010.** No real Deal Room content may be sent to a provider until OD-010 is decided | Unset it: no provider call is made; originals stay readable. |
| `DEAL_ROOM_TRANSLATION_PROVIDER` | **Proposed (LB-009), server-only, dev aid.** `test` selects the deterministic in-process adapter (no network, no content sent); anything else selects the real Anthropic adapter (itself inactive unless the boundary above is open) | Real adapter (inactive) | Not set | n/a - selection only, sends nothing while the boundary is closed. |

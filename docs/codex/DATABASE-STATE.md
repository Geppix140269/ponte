...
## The 29 tables the repository never described

Production holds **52 tables** in `public`. The repository declares **23** of
them, across `supabase/schema.sql` and `supabase/migrations`. The other **29**
were created straight against the database or pre-date the migration chain
entirely, and until 28 July 2026 nothing in this repository said they existed.

They are recorded in
`supabase/baseline/2026-07-28-undeclared-production-tables.sql`: 29 tables, 272
columns, 101 constraints, 68 indexes and 38 RLS policies, generated by reading
`pg_catalog`. Notable among them are the shop-era tables `products` (54 rows),
`categories` (10), `bundle_items` (33), `orders`, `order_items` and
`order_notes`, which `supabase/schema.sql` still describes as dropped by
`20260722a_drop_legacy_shop.sql`; that file is in `supabase/pending/` and has
never run. Also there: `deals`, `settlements`, `messages`, `organizations`,
`notifications`, `audit_logs`, `analytics_events`, `subscriptions`,
`saved_searches`, `listings_legacy_20260720` and `schema_migrations` itself.

**What the file is.** A photograph of what these tables ARE. It closes the
description gap, so an audit finally has something to diff against and a fresh
project can be brought to production's shape.

**What it is not.** It does not close the provenance gap. It does not say why
any of these tables exist, who decided them, or whether they are still wanted.
Four of them hold live rows.

**It is deliberately not in `supabase/migrations`.** Nothing applies it, it has
never been run, and it must not be added to the auto-apply chain without a
reconciliation plan and owner approval. Regenerate it rather than hand-editing
it, so that it keeps saying what production actually is.

**Grants are not emitted, and that is deliberate.** Supabase's default
privileges give `anon` and `authenticated` all seven table privileges on every
new table in `public`, and 51 of the 52 tables here carry exactly that. It is
not the hole it appears to be: RLS is the control, the grant only decides
whether PostgREST attempts the query, and all 51 have RLS enabled.
`schema_migrations` was the one table with RLS **disabled**, which is why the
same default grants made it publicly readable and writable, and why it is now
the one table with an explicit revoke. A default grant is harmless only while
RLS is on.

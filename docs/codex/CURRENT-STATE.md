...
### The schema and payment side, unchanged

**No billing record, entitlement kind, Stripe object, tax treatment or production charging exists.** The Deal Room launch slice carries `starter`, `sponsored` and `waived` entitlement kinds only; `deal_room_entitlements` has no `paid` value in its CHECK constraint and contains no price, currency, period price, purchased branch capacity or payment reference, and its migrations contain no price, currency, invoice or Stripe identifier of any kind. The only Stripe checkout in the repository sells credit packs.

**Corrected 31 July 2026.** This paragraph previously opened "No price, pricing engine, branch-counting predicate, billing record..." — the first three of those are no longer true, because Stage 2 delivered the pure engine and the branch-counting predicate the same day. The rest of the sentence stands, and the distinction is the one that matters: **a price can now be computed, and still cannot be charged.** Recording ADR-0020 changed nothing on its own; Stage 2 changed only what is calculable. The staged programme is `docs/plans/active/deal-room-transaction-pricing.md` and every production-changing stage in it needs separate owner approval.

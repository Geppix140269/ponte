...
| Landing bridge implementation | On `main` via PR #63, mobile overlap corrected by PR #65 | Not independently production-verified | The temporary family/action card grid is replaced by the approved Family Bridge and Action Bridge, drawn from the recovered engine's own cubic Bezier geometry. Product sectors and all HS copy removed from the landing by owner decision. All nine destinations preserved and asserted. Evidence: `docs/codex/audits/constitution-rebuild/evidence/landing-bridges/`. **Pre-measurement layout added 31 July 2026 (PL-032), after the bridge was observed collapsed on production.** A horizontal station is `position: absolute` with no coordinates in the approved stylesheet; every number that separates one from the next is written by the measuring effect. So when the client did not arrive, all three stations painted on one point and the stage - sized from its lowest child - kept a height of zero and let Market Signals draw through them. Reproduced and measured: three overlapping pairs, stage height 0, the section below overlapping the bridge by **139px**. `BridgeRoute` now ships `data-unmeasured` on the stage in the server HTML and removes it only once every station carries a real position, and `bridge-integration.css` lays the stations out in plain flow until then. The approved stylesheet is untouched (`check-bridge-invariance.mjs` green); `DealRoomBridge` renders the same `.br__stage` class, does not render the attribute, and is unaffected. **The settled rendering is provably unchanged**: the four desktop reference frames and two of the three mobile frames regenerate byte-identical against `main`, and `mobile-1-family-neutral-390x844.png` is bistable on `main` itself across consecutive runs on identical source. Fallback evidence: `no-client-chunks-1280.png` and `no-client-chunks-390.png` in the same folder. |

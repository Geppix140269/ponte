...
| Member-business verification is free (#135, ADR-0018) | **On `main`** via `8c1d856` | Deployed with the commits that followed it | Closes the P0 commercial defect the owner observed on 30 July 2026. `member_business` verification performs no balance lookup, no cost display, no top-up link, no HTTP 402, no credit spend and no ledger entry: the single `spendCredits` call in `lib/verification/pipeline.ts`, and the balance read plus the 402 reply in `app/api/verification/route.ts`, are guarded by a `paidPurpose` derived from the already-normalised purpose, so a free run leaves `credit_ledger_id` null. `app/[locale]/verify/page.tsx` reads a balance only for the paid path and passes `cost=null` otherwise; `components/VerifyForm.tsx` renders no balance, cost, shortfall or top-up when the check is free and states plainly that it is free. Signed-out gate copy is purpose-aware (`bodyBusiness`/`bodyCounterparty`), a defect caught by browser evidence rather than source review. `counterparty_check` keeps its paid rule and stays commercially separate. ADR-0018 records the durable decision; `lib/verification/__tests__/member-business-free.test.ts` (9 checks in `npm test`) fails if any guard is lost, but it reads source text only; the runtime proof is cutover PR 6, below. **The "no refund" claim was not true when this row was first written, and is now.** As shipped in `8c1d856` the refund at the bottom of `runLevel2Checks` was guarded by `if (userId)` and not by the purpose, so a free run whose checks threw called `refundSpend` and granted two credits back for a verification that never paid — the one real breach of ADR-0018, found by executing the pipeline in PR 6. PR 6 recorded the breach rather than fixing it and was merged that way, so `fc1b4cc` carried it to `main`. Fixed immediately after, on branch `claude/verification-refund-purpose-guard`: `runLevel2Checks` derives the same `paidPurpose` from the purpose it is handed and the refund is guarded `if (paidPurpose && userId)`, so a free run reaches no credit function on any path, failure included. A missing or unknown purpose still normalises to `counterparty_check`, so no paid run loses its refund. `npm run verify` green. **Still open on #135: the route renders the intermediate `PonteShell`, not `DeskShell`** (required outcome 3, separate follow-up PR). **Not deployed; signed-in acceptance outstanding.** |
| Single-generation product cutover — verification (PR 6) | **On `main`** via `fc1b4cc` (PR #181) | Deployed with the commits that followed it | Issue #130 cutover PR 6. **No behaviour changed by this PR itself; the defect it found was fixed by the follow-up recorded in the row above.** The commercial boundary this PR was scoped to build had already shipped under Issue #135 (ADR-0018): the `paidPurpose` guard in `lib/verification/pipeline.ts`, the `purpose !== "member_business"` guard around the balance read and the 402 in `app/api/verification/route.ts`, and the paid-only cost surface on `app/[locale]/verify/page.tsx`. What had not shipped was the proof ADR-0018 asks for, and that proof is all this PR added. `lib/verification/__tests__/member-business-free-runtime.test.ts` (10 tests in `npm test`) EXECUTES the real `runLevel2` pipeline and the real `POST /api/verification` handler against test doubles and asserts a `member_business` run records zero calls to any credit function, not only to `spendCredits`. The doubles are `lib/__mocks__/credits.ts` (records every credit function call by name and argument; remapped from `@/lib/credits` in `tsconfig.test.json`), plus `supabase-admin.ts`, `ai.ts` and `registry.ts`; `lib/__mocks__/supabase-server.ts` gained `single()`, `upsert()` and `rpc()`. The harness is required to SEE a real spend on the `counterparty_check` path and a real 402, because a double that failed to intercept would make every zero-call assertion pass for the wrong reason. The existing source-reading test is kept: it pins the readable shape of the guards, and both files now say what each proves and what it does not. **The proof found one real breach of ADR-0018, and this PR recorded it rather than fixing it: the refund in `runLevel2Checks` was guarded by `if (userId)` alone, so a free `member_business` run whose checks threw called `refundSpend(userId, COST_VERIFICATION_L2, id)` and granted two credits back for a run that never paid. It was pinned as a named KNOWN GAP test so the breach was visible in CI rather than only in a report — which is how it survived the merge in the open.** Fixed by the immediate follow-up on branch `claude/verification-refund-purpose-guard`: the refund now carries the same purpose guard as the spend (`if (paidPurpose && userId)`, with `paidPurpose` derived in `runLevel2Checks` via `normalizePurpose(opts.purpose)`), and the KNOWN GAP test becomes the zero-call assertion its own note described. Its paired falsifiability test — the same upstream fault on the `counterparty_check` path still refunds — is kept, so the guard cannot pass by having switched the refund off for everyone. The source-reading test gains a ninth check pinning the shape of the new guard, which is what makes the ADR-0018 consequence "fails if any credit call site on the shared path loses its purpose guard" true of the refund as well as the spend. **The follow-up is a money change and needs owner approval before merging.** `VERIFICATION_CREDIT_COUPLING` did not shrink: all four listed files still import `@/lib/credits` for the paid counterparty path, and moving a constant to clear the baseline would game the ratchet without decoupling anything. `npm run verify` green. **Not deployed.** |
...
| **Deal Room-Only Pricing Authority (ADR-0020)** | **On `main`.** Authority via PR #155 (`7aba8ed`), reconciliation via PR #160 (`5e029ef`), operations log via PR #162 (`39a7140`), all 31 July 2026 | **Not started. No production action of any kind** | Owner decision of 31 July 2026: the Deal Room is Ponte's only paid product, **$79 USD for 30 active days**, five concurrently active private principal-counterparty Deal Branches included, **$15 USD** per additional concurrent branch, capped at **$199 USD** per Master Deal Room per 30-day period, **USD only**, five languages included. Supersedes Starter access, Portfolio subscriptions, Ponte Credits, credit-funded rooms, paid verification, paid verification badges, public Ponte Desk packages, retainers, success fees, commissions, percentage-of-transaction pricing, euro Deal Room prices and unlimited free principal-counterparty branches. **Recording the decision is not implementing it.** Programme: `docs/plans/active/deal-room-transaction-pricing.md`. Inventory: `docs/codex/audits/deal-room-pricing/INVENTORY-2026-07-31.md`. **Both owner decisions are settled, 31 July 2026.** **OD-012** (Amendment 1): a broker's branch counts. **OD-011** (Amendment 2): the private counterparty check stays paid, and §1 and §15 are amended so the authority stops prohibiting it; Ponte Credits stay retired, so it must be repriced in USD. |
| **Expiry and reactivation (programme Stage 5)** | **On branch `claude/deal-room-expiry-reactivation`** (31 July 2026, owner authorised) | **Not started. Nothing imports it; no room expires or reactivates today** | `lib/deal-room/period-lifecycle.ts`, pure, enforcing authority §12's "the commercial record must never be held hostage behind a continuing subscription". **`RoomAccess.readable` has no `false` case** — a test walks every room-period state against five instants and the no-period case and asserts readability every time. Writability requires **both** the `active` state and the window, so a row left `active` past its end does not keep a room writable. `PRESERVED_ON_EXPIRY` lists §12's nine survivals as asserted data; `CHANGED_ON_EXPIRY` has exactly one entry with its length pinned. `RENEWAL_POLICY` records that renewal is deliberately manual rather than merely unbuilt. `reactivationQuote` prices the branches **resumed**, not what the old period carried, and resuming zero costs nothing (`payable: false`). `branchActivationNeedsPayment` counts branches **concurrently, never cumulatively**, so a released slot is reusable inside the paid period at no charge (§7). A test asserts the module exports no `delete`, `remove`, `revoke`, `purge`, `destroy` or `erase` function. **30 assertions**, proved to fail in four directions. |
| **Charging decisions (programme Stage 4a)** | **On branch `claude/deal-room-charging-decisions`** (31 July 2026, owner authorised) | **Charging is OFF. Ponte has never taken a Deal Room payment, and cannot** | `lib/deal-room/charging.ts`: pure, no Stripe SDK import, no clock, `process.env` read only inside the gate. **The gate requires four conditions and has none of them** — `DEAL_ROOM_BILLING=on` (server-only, so unreadable and unsettable from a browser), `NEXT_PUBLIC_DEAL_ROOM=on`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`. The fourth gates **checkout**, not only fulfilment: a charge that could not be verifiably confirmed must never be started. Three server-priced checkout decisions (activation, additional branch, reactivation) — **no function accepts an amount from its caller**, asserted structurally — and one fulfilment decision covering wrong event type, unpaid session, **replay checked before anything can grant**, orphaned payment (loud, never retried), unknown kind, amount mismatch in both directions, a branch charge against a period at the cap, and a charge that would take the period total past $199. **34 assertions** in `lib/deal-room/__tests__/charging.test.ts`, including that a branch charge's wording names no counterparty or competitor (§10). **No route, no Stripe object, no key.** Stage 4b (the HTTP surfaces) is deliberately held until the migration is applied and the Stripe gates are being opened — see the plan, §12 discovery 6. |
| **Paid room periods and billing events (programme Stage 3)** | **Migration WRITTEN and NOT APPLIED**, on branch `claude/deal-room-pricing-billing-records` (31 July 2026, owner authorised) | **Not started. Executed nowhere; nothing reads it; no charge** | `supabase/migrations/20260731e_deal_room_paid_room_periods.sql`, SHA-256 `3456e0b0...ff78a422`, 15,752 bytes. Additive: `paid` as a fourth `deal_room_entitlements.kind` (three existing values preserved, no backfill), `deal_room_room_periods`, `deal_room_billing_events`, `deal_room_entitlements.current_period_id`. **Three invariants enforced by the database:** price CHECKed against the authority's own formula so a row whose price does not follow from its capacity cannot be written; the $199 cap stated again independently; one active period per room via a partial unique index. `amount_due_cents` is a stored generated column so a §17 waiver cannot drift from the list price it waives; `state <> 'active' or confirmed_at is not null` makes §9 a constraint. **Replay safety:** `provider_event_id` unique where not null, plus an append-only trigger that refuses UPDATE and DELETE to the table owner. **Read policy is administrator-only** (`deal_room_can_administer`), narrower than every other member-facing Deal Room table, because `purchased_branch_capacity` is a branch-count disclosure under §4 and §11; no member holds any write policy; `anon` and `authenticated` revoked then `authenticated` re-granted SELECT alone (LB-008 lesson). `lib/deal-room/billing.ts` is the pure TypeScript half; **35 assertions** in `lib/deal-room/__tests__/billing.test.ts`, proved to fail on a drifted cap, a widened policy and a removed idempotency index. **Applying the migration is a separate owner approval.** `STARTER_LIMITS_PROPOSED` was deliberately **not** removed — it has two live member-facing call sites, so its retirement is Stage 6. |
| **Pricing engine and branch-counting contract (programme Stage 2)** | **On `main`** (31 July 2026, owner authorised) | **Not started. Nothing charges, and nothing imports the module** | `lib/deal-room/pricing.ts`: a **pure** module — no database, network, clock or environment, one type-only import — with the six canonical constants, `roomPeriodPriceCents()`, `additionalBranchChargeCents()`, the $199 cap, `formatUsd()` (always writes `USD`), `PUBLISHED_PRICE_TABLE` pinned against the engine, and `branchBillingVerdict()` mapping authority §7's five conditions onto `deal_room_sub_rooms.kind`/`.state`, `deal_room_participants.participant_class`/`.state` and the four-agreement admission gate, returning a **named reason** on refusal. `lib/deal-room/__tests__/pricing.test.ts`, **50 assertions** in `npm test`, covering every §7 inclusion and exclusion, all eight sub-room states, purity, and non-disclosure (prices take a **count**; the facts types are held to a **field allowlist** so no identifier can be added silently). **Proved to fail in three directions** before acceptance — cap raised, provider workspaces made billable, identifier added. The third initially **passed and should not have**: the check scanned for `ref:` and `subRoomRef` walked past it on the capital R, which is why it is an allowlist now. A test asserts **nothing under `app/` or `components/` imports the module**, so wiring it is a deliberate later-stage act. **No schema, route, UI, Stripe object or charge.** |
...
| **`/pricing` rebuilt to one product and one formula (programme Stage 6)** | **On branch `claude/stage-6-pricing-surface`** (31 July 2026, owner authorised) | **Not deployed. Deployed state of the OLD page unknown; the deployed commit is unrecorded (see the note below this table)** | The four-panel `auto-fit` grid — free marketplace, **Credits**, **Desk success fee**, **Desk retainer** — is **gone rather than refilled**: §19 forbids a multi-plan comparison grid, so the shape changed with the words. One product panel (**Ponte Deal Room, $79 USD, 30 active days**), the formula as three plain sentences plus a worked example, then included / free / never-charged / expiry. The Deal Room appears on the fees page for the first time. The **footer blurb** stops carrying "success fee or retainer" onto every page; **two `/about` paragraphs** corrected; the **route manifest** no longer names a retired engagement (**closes PL-038**). The counterparty check is named without a number — it stays paid (OD-011) but is still credit-priced until Stage 8, and the page may state neither a credit price (§15 prohibits the currency) nor a USD price (none exists yet). **Finding:** `POST /api/credits/checkout` has **no caller anywhere** and no component uses `CREDIT_PACKS` — credits were describable but **not buyable through the interface**, so removing the panel stranded no purchase path. **LB-014 stays open:** the owner has never classified it, and the Design Constitution evidence (desktop, 390×844, reduced motion, design approval) **was not captured** — `middleware.ts` gates the site behind Basic auth and the password is not in the repository; the gate was **not** weakened, and the owner authorised proceeding with the evidence marked outstanding. Static conformance checked: every class used is defined in the approved `desk.css`, no new component, token, icon or motion. The `/about` **legal-entity** sentence is deliberately unchanged — it is a statement about two incorporated entities and the wording is the owner's. |
...
| Landing bridge implementation | On `main` via PR #63, mobile overlap corrected by PR #65 | Not independently production-verified | The temporary family/action card grid is replaced by the approved Family Bridge and Action Bridge, drawn from the recovered engine's own cubic Bezier geometry. Product sectors and all HS copy removed from the landing by owner decision. All nine destinations preserved and asserted. Evidence: `docs/codex/audits/constitution-rebuild/evidence/landing-bridges/`. **DS-10, 31 July 2026:** the horizontal bridge is positioned entirely by a layout effect and the approved stylesheet gives `.brst` `position: absolute` with no coordinates, so until that effect ran every station landed on one static position and the stage kept zero height. Seen live in Chrome after the deploy of `f26718a` and self-recovered; most likely a client chunk that failed mid-deploy. The `<noscript>` rule covered "scripting is off", not "the script has not run yet". Section 5 of `components/ponte/bridge/bridge-integration.css` now gives an unmeasured stage a flow layout and a `DECK_HEIGHT` minimum; `BridgeRoute` sets `data-measured` on the stage inside the same effect, after positioning and before `fit()`, and every fallback selector is guarded by `:not([data-measured])`, so the settled rendering is unchanged (0 computed-style differences on the stage, a station and both abutments at 1280 and 390). Regression checks in `lib/landing/__tests__/family-action-bridges.test.tsx` (in `npm run verify`) and `e2e/landing-bridges.spec.ts` (JS disabled, 1280 and 390). The Playwright suite was **not run**: it needs the temporary Basic-auth site gate's password, which is not in the repository. |
...
### Implementation status: the engine exists and is wired to nothing

**Stage 2 of the pricing programme is on `main`** (31 July 2026, owner
authorised). `lib/deal-room/pricing.ts` is a **pure** module — no database, no
network, no clock, no environment, one type-only import — holding the six
canonical constants, `roomPeriodPriceCents()`, `additionalBranchChargeCents()`,
the $199 cap, `formatUsd()` (always writes `USD`), the published price table,
and `branchBillingVerdict()`: authority §7's five conditions mapped explicitly
onto `deal_room_sub_rooms.kind` and `.state`,
`deal_room_participants.participant_class` and `.state`, and the four-agreement
admission gate, returning a **named reason** on refusal.
`lib/deal-room/__tests__/pricing.test.ts` holds **50 assertions** in `npm test`
and was proved to fail in three directions before acceptance.

**Nothing imports it, and the suite asserts that.** Wiring it is a later stage,
so a caller appearing without owner approval fails the build rather than
shipping.

**Everything else is still absent.** No billing record, no `paid` entitlement
kind (`deal_room_entitlements.kind` still admits only `starter`, `sponsored` and
`waived`, and the table holds no price, currency, period price, purchased branch
capacity or payment reference), no Stripe object for a Deal Room, no pricing
surface, no multilingual billing notice, no charge. **Ponte cannot take money
for a Deal Room.** The live `/pricing` page still publishes the retired model
(LB-014, proposed).

**OD-012 is decided (31 July 2026): a broker's branch counts.** Authority §7
said "principal-counterparty" while §4 gave "a broker acting for a disclosed or
controlled principal" as an example of a branch; the owner resolved it toward §4
and directed that the authority be amended, which it now is (**Amendment 1**).
No behaviour changed — `BILLABLE_PARTICIPANT_CLASSES` already held
`["principal", "intermediary"]` pending the decision, pinned by a named test.

### The schema and payment side

**In production, unchanged: no billing record, no `paid` entitlement kind, no Stripe object for a Deal Room, no tax treatment and no charging.** `deal_room_entitlements` in production still admits only `starter`, `sponsored` and `waived` and holds no money. The only Stripe checkout in the repository sells credit packs.

**In the repository, Stage 3 added a migration that is written and NOT applied.** `supabase/migrations/20260731e_deal_room_paid_room_periods.sql` (SHA-256 `3456e0b0...ff78a422`) adds `paid` as a fourth entitlement kind, `deal_room_room_periods`, `deal_room_billing_events` and `deal_room_entitlements.current_period_id`. It is executed nowhere, applying it is a separate owner approval, and **no file under `app/`, `components/` or `lib/` reads either new table** — asserted by test, so a reader cannot ship ahead of the migration. Full record, including the three database-enforced invariants and the administrator-only read policy: `docs/codex/DATABASE-STATE.md`.

`lib/deal-room/billing.ts` is its pure TypeScript half (vocabularies, `periodEndFrom`, `amountDueCents`, `draftRoomPeriod`, `launchPartnerWaiver`), with **35 assertions** in `lib/deal-room/__tests__/billing.test.ts` that compare the SQL price formula against the pricing engine at every capacity from 5 to 40.

**Corrected 31 July 2026, twice in one day.** This paragraph previously opened "No price, pricing engine, branch-counting predicate, billing record..." Stage 2 made the first three false and Stage 3 made the fourth false *in the repository only*. The distinction is the one that matters and is easy to lose: **a price can now be computed, and a billing record can now be described, and Ponte still cannot charge anybody.** The staged programme is `docs/plans/active/deal-room-transaction-pricing.md`; every production-changing stage in it needs separate owner approval.
...
8. ~~Decide **OD-011**.~~ **Decided 31 July 2026: the counterparty check stays paid** (authority Amendment 2, amending §15 and §1). Ponte Credits are still retired, so the check must be repriced as a direct USD charge. Stage 8 is unblocked.

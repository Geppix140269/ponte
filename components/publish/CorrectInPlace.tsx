...
/**
 * Correction in place: a sheet of rows over the fact list, never a bare field.
 *
 * ## Why a sheet and not an input
 *
 * A bare text field asks the member to already know the vocabulary. "Incoterms"
 * with a cursor in it is answerable only by someone who can spell DPU; a list
 * of eleven rules with what each one means is answerable by everyone, and it is
 * the same eleven values either way. Typing stays available inside the sheet
 * for the cases a list cannot cover, and is never the only route.
 *
 * ## Why the quantity gets a stepper
 *
 * Because a number is the one value a list cannot enumerate. The stepper is
 * tap-first with the figure between two 72px controls, and the mode ("tonnes
 * per month", "containers per month") is chosen from rows beneath it. The
...
export interface CorrectInPlaceProps {
...
export default function CorrectInPlace({ field, draft, onDraft, onClose }: CorrectInPlaceProps) {
    ...
    <>
      ...
      */}
      <button
        ...
        onClick={onClose}
      />
      <div className="pick" role="dialog" aria-label={FIELD_LABEL[field]}>
        ...
        {field === "quantity" && (
          <>
            <div className="step2">
              <button type="button" aria-label="Less" onClick={() => setAmount((n) => Math.max(1, n - 1))}>
                &minus;
              </button>
              <span>{amount}</span>
              ...
              <div className="rows">
                ...
                */}
                ...
                >
                  ...
                  <span>
                    <b>On request</b>
                    <small>You have not fixed a figure. That is an answer.</small>
                  </span>
                  <span className="go">›</span>
                </button>
              </div>
              ...
              <div key={group.label ?? index}>
                ...
                <div className="rows">
                  {group.options.map((option) => (
                    ...
                    >
                      <span className={`mk${chosen === option ? " mk--done" : ""}`} />
                      <span>
                        <b>{option}</b>
                      </span>
                      <span className="go">{chosen === option ? "" : "›"}</span>
                    </button>
                  ))}
                </div>
              </div>

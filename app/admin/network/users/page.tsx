...
export default async function NetworkUsers() {
  ...
  return (
    <div>
      <h1 className="serif text-ink mb-2" style={{ fontSize: 28, fontWeight: 500 }}>Users</h1>
      ...
      <div className="space-y-3">
        {rows.map((u: any) => (
          <div key={u.id} className="glass p-5 flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-ink">{u.full_name ?? "—"} {u.company ? `· ${u.company}` : ""} {u.verified_trader ? <span className="badge-gold ml-2">Verified</span> : null}</p>

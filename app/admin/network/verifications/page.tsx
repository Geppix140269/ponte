...
export default async function VerificationsQueue() {
  ...
  return (
    <div>
      <h1 className="serif text-white mb-2" style={{ fontSize: 28, fontWeight: 500 }}>Verification queue</h1>
      ...
      <div className="space-y-3">
        {rows.map((v: any) => (
          <div key={v.id} className="glass p-5 flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-white">{v.profile?.full_name ?? "—"} {v.profile?.company ? `· ${v.profile.company}` : ""}</p>

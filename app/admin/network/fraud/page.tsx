...
export default async function FraudQueue() {
  ...
  return (
    <div>
      <h1 className="serif text-white mb-2" style={{ fontSize: 28, fontWeight: 500 }}>Fraud queue</h1>
      ...
      <div className="space-y-3">
        {rows.map((f: any) => (
          <div key={f.id} className="glass p-5 flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-white">{f.flag_type.replace(/_/g, " ")} <span className="badge ml-2">{f.severity}</span></p>

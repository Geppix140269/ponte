    ...
    <>
      ...
      <section className="container-px py-20">
        <div className="glass p-10 max-w-md mx-auto">
          ...
          <h1 className="serif text-white mt-6 mb-2" style={{ fontSize: 36, fontWeight: 500 }}>{t("title")}</h1>
          <p className="text-gray-2 text-[14px] mb-7">{t("subtitle")}</p>
          ...
          {!configured ? (
            ...
            <div className="flex items-center gap-2 rounded-[10px] px-4 py-3 text-[13px] text-positive" style={{ background: "rgba(74,192,154,0.15)", border: "1px solid rgba(74,192,154,0.35)" }}>
              <Mail className="h-4 w-4" /> {t("checkInbox")}
            </div>
          ) : (
            <div className="space-y-5">
              {GOOGLE_CLIENT_ID ? (
                ...
                <div className="glass-tight p-4 text-[12px] text-gray-2">{t("googleDisabled")}</div>
              )}
              ...
              <form onSubmit={signInWithEmail} className="space-y-3">
                <div>
                  ...
                  <input id="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="field" placeholder={t("emailPlaceholder")} />
                </div>

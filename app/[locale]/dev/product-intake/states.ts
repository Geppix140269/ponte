...
import { groundIdentification } from "@/lib/products/identify";
import type { IdentifiedProduct, ProductCandidate, ResolutionOutcome } from "@/lib/products/model";
import { bandFor } from "@/lib/products/model";
...
/*
 * The identified-product states.
 *
 * Built from the SAME grounding function the live cascade uses, with a stand-in
 * customs catalogue holding only real HS 2022 codes, so these frames show what
 * a member actually sees rather than a hand-drawn approximation of it. The
 * model answer below is the shape the identification prompt asks for.
 */
const HS_STUB: Record<string, string> = {
  "080440": "Fruit, edible; avocados, fresh or dried",
  "081190": "Fruit and nuts; frozen, other",
  "151590": "Fixed vegetable fats and oils; other",
};
const hsStub = async (code: string) => (HS_STUB[code] ? { code, description: HS_STUB[code] } : null);

const AVOCADO_ANSWER = {
  isProduct: true,
  products: [
    {
      name: "Avocado, fresh",
      ...
      group: "Fresh fruit",
      form: "fresh",
      distinguisher: "The fresh fruit, sold by count and size grade and shipped chilled.",
...
async function identifiedCandidates(correction: string | null): Promise<ProductCandidate[]> {
  const answer = correction
    ? { ...AVOCADO_ANSWER, products: [{ ...AVOCADO_ANSWER.products[0], correction }, ...AVOCADO_ANSWER.products.slice(1)] }
    : AVOCADO_ANSWER;
  const grounded = await groundIdentification(answer, hsStub);
  return grounded.products.map((entry, index) => {
    const score = Math.max(0.3, Math.min(0.88, entry.confidence * 0.9 - index * 0.06));
    const product = entry.product as IdentifiedProduct;
    return {
      product,
      score: Number(score.toFixed(3)),
      band: bandFor(score),
      matchedOn: [
        { kind: "semantic" as const, term: product.correction ? `read as "${product.correction}"` : "identified from your description" },
      ],
    };
  });
}

const identifiedOutcome = (candidates: ProductCandidate[], wording: string): ResolutionOutcome => ({
  kind: "candidates",
  candidates,
  wording,
});

async function lowConfidenceCandidates(): Promise<ProductCandidate[]> {
  const grounded = await groundIdentification(
    {
      isProduct: true,
      products: [
        {
          name: "Industrial fastener, unspecified",
          sector: "metal",
          group: "Hardware",
          distinguisher: "Ponte could not narrow this further from the words given.",
          confidence: 0.3,
        },
      ],
    },
  ...
  return grounded.products.map((entry) => ({
    product: entry.product,
    score: 0.27,
    band: bandFor(0.27),
    matchedOn: [{ kind: "semantic" as const, term: "identified from your description" }],
  }));
}
 ...
 * Every state, built.
 *
 * A function rather than a constant because the identified states are grounded
 * through the same async path the live cascade uses, and grounding a customs
 * code is a lookup. The page is a server component and awaits this.
 */
export async function gallery(): Promise<GalleryState[]> {
  const avocadoCandidates = await identifiedCandidates(null);
  const avogadoCandidates = await identifiedCandidates("avocado");
  const lowConfidence = await lowConfidenceCandidates();
  ...
  return [
    ...
    {
      id: "identified",
      title: "Identified, outside the curated catalogue",
      note: "The owner's review case. `avocado` is identified, ranked and offered, and says it is not yet confirmed.",
      session: drive({ type: "chooseMethod", method: "describe" }, { type: "resolution", outcome: identifiedOutcome(avocadoCandidates, "avocado") }),
    },
    {
      id: "corrected",
      title: "Spelling corrected",
      note: "`avogado` asks Did you mean avocado? The correction is offered, never applied silently.",
      session: drive({ type: "chooseMethod", method: "describe" }, { type: "resolution", outcome: identifiedOutcome(avogadoCandidates, "avogado") }),
    },
    {
      id: "low-confidence",
      title: "Low-confidence identification",
      note: "Offered, and marked as a guess, rather than withheld or presented as settled.",
      session: drive({ type: "chooseMethod", method: "describe" }, { type: "resolution", outcome: identifiedOutcome(lowConfidence, "those metal things") }),
    },
    {
      id: "identified-review",
      title: "Review of an identified product",
      note: "Provenance reads Identified by Ponte, not yet confirmed. Confirming is what changes it.",
      session: drive(
        { type: "chooseMethod", method: "describe" },
        { type: "resolution", outcome: identifiedOutcome(avocadoCandidates, "avocado") },
        { type: "chooseCandidate", productKey: avocadoCandidates[0]?.product.key ?? "" },
      ),
    },
    {
      id: "unmatched",
      title: "Not recognised",
      note: "Ponte's limit, stated without blaming the member, with four ways on. Corrected at owner review.",
      session: drive(
        { type: "chooseMethod", method: "describe" },
